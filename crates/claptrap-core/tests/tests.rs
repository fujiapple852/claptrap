...
use claptrap_core::OutputFormat;
...
fn run(spec: &str, args: &str) -> String {
    ...
    claptrap_core::parse(app, args).render(OutputFormat::Posix)
}

mod basic {
    use claptrap_core::{CatCmd, ExitCode, Output, OutputFormat};
    ...
    fn cat_cmd_handles_eof_in_message() {
        ...
        let cmd = Output::Cat(CatCmd::new(
            styled,
            ExitCode::Error,
            clap::ColorChoice::Never,
        ))
        .render(OutputFormat::Posix);
        snapshot!(format!("{}", cmd));
    }
}
...
mod arg {
    ...
    #[test_case(case!("arg", "long-1"))]
    #[test_case(case!("arg", "long-2"))]
