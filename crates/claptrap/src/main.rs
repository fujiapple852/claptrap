...
use clap::Parser;
use claptrap_core::template_values;
use claptrap_core::Command;
use claptrap_core::{CatCmd, ExitCode, Output};
use cli::{DocFormat, OutputFormat, Shell, SpecFormat, SpecInfo, SubCommand};
    ...
    match cli.command {
        ...
        None => {
            ...
            match panic::catch_unwind(AssertUnwindSafe(|| {
                ...
                Ok(val) => match val {
                    Ok(output) => {
                        write!(stdout, "{}", output.render(cli.output_format.into()))?;
                        stdout.flush()?;
                        exit(0);
                    }
                    Err(err) => {
                ...
                Err(err) => {
                    ...
                    write!(stdout, "{}", panic.render(cli.output_format.into()))?;
                    stdout.flush()?;
                    exit(0);
                }
            }
...
fn run_app(
    ...
    Ok(claptrap_core::parse(spec, args))
}
...
impl From<OutputFormat> for claptrap_core::OutputFormat {
    fn from(format: OutputFormat) -> Self {
        match format {
            OutputFormat::Posix => Self::Posix,
            OutputFormat::Fish => Self::Fish,
            OutputFormat::PowerShell => Self::PowerShell,
        }
    }
}
