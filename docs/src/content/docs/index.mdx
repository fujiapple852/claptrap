...
<CardGrid>
	<Card title="Powered by Clap" icon="rocket">
	    Claptrap brings the [full power](https://claptrap.cli.rs/reference/supported/) of [Clap](https://crates.io/crates/clap) command line parsing to shell scripts.
	</Card>
    <Card title="Declarative Specification" icon="rocket">
...
A simple `toml` spec embedded in a bash script:
...
A standalone `toml` spec with subcommands:
...
```
  ...
  A standalone spec written in `yaml`:
...
$ claptrap completion --spec myapp.json --shell zsh -o _myapp.zsh
...
Generate JSON schema for the spec format:
